// Verbatim copy of `supabase/functions/import-worker/parser.ts`, which is
// itself a port of `apps/web/src/import/llm.ts` parseLlmJson plus the tiny
// slice of `@cookyourbooks/domain` it depends on. Edge functions are
// bundled per-directory on deploy, so the parser is copied in rather than
// imported across function dirs. Behaviour is bit-exact with both copies —
// when you fix a bug here, fix it in the other two too. Tests live next to
// the web copy.
