...
export interface Recipe {
  ...
  /**
   * Storage path (in the public `covers` bucket) of this recipe's cover
   * image — uploaded by the user or generated by the Gemini cover worker.
   * The public URL is derived via `supabase.storage.from('covers')
   * .getPublicUrl(path)`. Same path convention as a collection's
   * `coverImagePath`.
   */
