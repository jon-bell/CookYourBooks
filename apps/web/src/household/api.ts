...
// The household RPCs / tables aren't in the committed database.types.ts
// yet (that file is regenerated by hand and held back until a release
// pass). At runtime PostgREST knows about them, so we go through `any`
// at the API boundary and re-impose typed interfaces on the way out.
//
...
export async function getMyHousehold(): Promise<{
  ...
  if (!user) return null;
  const { data: memberRows, error: memberErr } = await untyped()
  ...
  const { data: householdRow, error: hhErr } = await untyped()
    .from('households')
    .select('*')
  ...
  const memberIds = (memberRows as HouseholdMember[])
    ...
    .map((m) => m.user_id);
  const { data: profileRows } = await untyped()
    .from('profiles')
...
export async function previewInvite(token: string): Promise<HouseholdInvitePreview | null> {
  const { data, error } = await untyped().rpc('preview_household_invite', { p_token: token });
  if (error) throw error;
...
export async function createHousehold(name: string): Promise<string> {
  const { data, error } = await untyped().rpc('create_household', { p_name: name });
  if (error) throw error;
  return data as string;
}

export async function renameHousehold(householdId: string, name: string): Promise<void> {
  const { error } = await untyped().rpc('rename_household', {
...
export async function deleteHousehold(householdId: string): Promise<void> {
  const { error } = await untyped().rpc('delete_household', { p_household_id: householdId });
  if (error) throw error;
}

export async function inviteToHousehold(householdId: string): Promise<string> {
  const { data, error } = await untyped().rpc('invite_to_household', {
    p_household_id: householdId,
  });
  if (error) throw error;
  return data as string;
...
export async function revokeHouseholdInvite(inviteId: string): Promise<void> {
  const { error } = await untyped().rpc('revoke_household_invite', { p_invite_id: inviteId });
  if (error) throw error;
}

export async function acceptHouseholdInvite(token: string): Promise<string> {
  const { data, error } = await untyped().rpc('accept_household_invite', { p_token: token });
  if (error) throw error;
  return data as string;
}

export async function leaveHousehold(): Promise<void> {
  const { error } = await untyped().rpc('leave_household');
  if (error) throw error;
}

export async function removeHouseholdMember(userId: string): Promise<void> {
  const { error } = await untyped().rpc('remove_household_member', { p_user_id: userId });
  if (error) throw error;
}

export async function transferHouseholdOwnership(newOwnerId: string): Promise<void> {
  const { error } = await untyped().rpc('transfer_household_ownership', {
...
export async function shareCollectionWithHousehold(
  ...
  const { error } = await untyped().rpc('share_collection_with_household', {
...
export async function unshareCollectionFromHousehold(collectionId: string): Promise<void> {
  const { error } = await untyped().rpc('unshare_collection_from_household', {
    p_collection_id: collectionId,
  });
  if (error) throw error;
}
...
export async function attestPublicShare(
  ...
  const { error } = await untyped().rpc('attest_public_share', {
    p_collection_id: collectionId,
    p_attestation: attestation,
  });
  if (error) throw error;
...
export async function acceptTos(version: number = CURRENT_TOS_VERSION): Promise<void> {
  const { error } = await untyped().rpc('accept_tos', { p_version: version });
  if (error) throw error;
}
