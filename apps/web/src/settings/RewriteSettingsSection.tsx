...
} from './rewriteSettings.js';

/**
 * Settings UI for the LLM-powered instruction rewriter. The user picks
 * a default model + prompt; the worker uses those when "Improve
 * instructions" runs on a recipe. The actual API key is reused from
 * the OCR Vault keys above this section.
 */
export function RewriteSettingsSection() {
  const [loaded, setLoaded] = useState(false);
  const [provider, setProvider] = useState<RewriteProvider>('gemini');
  const [model, setModel] = useState(DEFAULT_REWRITE_MODEL_BY_PROVIDER.gemini);
  const [prompt, setPrompt] = useState(DEFAULT_REWRITE_PROMPT);
  const [saved, setSaved] = useState(false);
  const [error, setError] = useState<string | undefined>();

  useEffect(() => {
    let cancelled = false;
    void getUserRewritePrefs()
      .then((p) => {
        if (cancelled || !p) {
          setLoaded(true);
          return;
        ...
        setModel(p.model || DEFAULT_REWRITE_MODEL_BY_PROVIDER[p.provider]);
  ...
  function onProviderChange(next: RewriteProvider) {
    setProvider(next);
    if (model === DEFAULT_REWRITE_MODEL_BY_PROVIDER[provider] || !model) {
      setModel(DEFAULT_REWRITE_MODEL_BY_PROVIDER[next]);
    }
    setSaved(false);
  }
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setError(undefined);
    try {
      await setUserRewritePrefs({ provider, model: model.trim(), prompt });
      setSaved(true);
    } catch (e) {
      setError((e as Error).message);
    }
  ...
  return (
    ...
    >
      ...
      <Field label="Provider">
        <select
          value={provider}
          onChange={(e) => onProviderChange(e.target.value as RewriteProvider)}
          disabled={!loaded}
